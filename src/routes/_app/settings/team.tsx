...
function InviteModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-50 w-[560px] bg-white shadow-2xl flex flex-col">
        {/* Header */}
        <div className="px-8 py-6 border-b border-[#e5e7eb] flex items-start justify-between gap-4">
          <div>
            <h3 className="text-[18px] font-semibold text-[#2e3240]">Invite new team member</h3>
            <p className="text-[13px] text-[#73767f] mt-1">Invite a new user to your organisation.</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-8 py-7 space-y-6">
          <div>
            <label className="block text-[14px] font-medium text-[#2e3240] mb-2">Email</label>
            <input
              type="email"
              value={email}
              ...
              autoFocus
              className="w-full px-4 py-3 border border-[#d1d5db] rounded-[8px] text-[14px] text-[#2e3240] placeholder-[#9fa1a7] focus:outline-none focus:border-[#0b66e4] focus:ring-1 focus:ring-[#0b66e4]"
            />
          </div>

          <div>
            <label className="block text-[14px] font-medium text-[#2e3240] mb-3">Role</label>
            <div className="flex flex-col gap-3">
              {(['admin', 'viewer'] as const).map((r) => (
                <label key={r} className="flex items-start gap-3 cursor-pointer">
                  ...
                  />
                  <div>
                    <p className="text-[14px] font-medium text-[#2e3240] capitalize">{r}</p>
                    <p className="text-[12px] text-[#73767f]">
                      {r === 'admin'
                        ? 'Can manage sites and users within the organisation'
                        : 'Can only view the dashboard and download scan reports.'}
                    </p>
                  </div>
                </label>
              ))}
          ...
          <div>
            <label className="block text-[14px] font-medium text-[#2e3240] mb-2">Organisation name</label>
            <div className="relative">
              <select
                value={orgId}
                onChange={(e) => setOrgId(e.target.value)}
                className="w-full px-4 py-3 border border-[#d1d5db] rounded-[8px] text-[14px] text-[#2e3240] focus:outline-none focus:border-[#0b66e4] bg-white appearance-none"
              >
              ...
              <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#9fa1a7] pointer-events-none" />
            </div>
          </div>
        </div>

        {/* Footer */}
        <div className="px-8 py-5 border-t border-[#e5e7eb] flex gap-3 justify-end">
          <button
            onClick={onClose}
            className="px-6 py-2.5 border border-[#d1d5db] rounded-[8px] text-[14px] font-medium text-[#73767f] hover:bg-[#f5f7fa] transition-colors"
          >
            Cancel
          </button>
          <button
            ...
            className="px-6 py-2.5 bg-[#0b66e4] hover:bg-[#0952c6] disabled:opacity-50 text-white text-[14px] font-medium rounded-[8px] transition-colors"
          >
