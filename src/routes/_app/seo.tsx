...
import { AlertTriangle, Loader2, AlertCircle, Search, ChevronLeft, ChevronRight } from 'lucide-react'
...
function SeoPage() {
  ...
  const { data: scoreData } = useQuery({
    queryKey: ['seo-score', scanId, strategy],
    queryFn: () => getSeoScore(scanId!, strategy),
    enabled: !!scanId,
  })

  const { data: criticalData } = useQuery({
    queryKey: ['seo-critical', scanId, strategy],
    queryFn: () => getSeoCriticalIssues(scanId!, strategy),
    enabled: !!scanId,
  })

  const { data: scoreOverTime } = useQuery({
    queryKey: ['seo-score-over-time', scanId, strategy],
    queryFn: () => getSeoScoreOverTime(scanId!, strategy),
    enabled: !!scanId,
  })

  const { data: affectedPages } = useQuery({
    queryKey: ['seo-affected', scanId, affectedPage, search, strategy],
    queryFn: () => getSeoAffectedPages(scanId!, affectedPage, 10, search, strategy),
    enabled: !!scanId && activeTab === 'Affected pages',
  })

  const { data: issueList } = useQuery({
    queryKey: ['seo-issue-list', scanId, issueListPage, strategy],
    queryFn: () => getSeoIssueList(scanId!, issueListPage, 10, strategy),
    enabled: !!scanId && activeTab === 'Issues list',
  })

  const { data: dashIssues } = useQuery({
    queryKey: ['seo-dash-issues-log', scanId, strategy],
    queryFn: () => getSeoIssuesLog(scanId!, FREE_PLAN_PREVIEW_ROWS, strategy),
    enabled: !!scanId,
  })
  ...
  if (!scanId) return <EmptyState msg="No completed scans yet. Run a scan to see SEO data." />
              ...
              <div className="flex flex-col justify-between lg:w-[30%] min-w-0">
                <div>
                  <h2 className="text-[22px] font-semibold text-[#2e3240] tracking-[-0.48px] leading-[1.4] mb-4">
                    Optimise Your Visibility: SEO Mastery
                  </h2>
                  ...
                  <ResponsiveContainer width="100%" height={240}>
                    <AreaChart data={chartData} margin={{ top: 5, right: 10, bottom: 5, left: -20 }}>
                      ...
                      <Area type="monotone" dataKey="score" stroke="#0b66e4" strokeWidth={2}
                        fill="url(#seoGrad)" dot={{ fill: '#06387d', r: 3 }} activeDot={{ r: 5 }} />
                    </AreaChart>
                  </ResponsiveContainer>
                )}
              </div>
      ...
      {activeTab === 'Affected pages' && (
        <div className="flex-1 p-3 sm:p-6 space-y-4">
          ...
          <div className="bg-white rounded-[8px] border border-[#9db7f4] p-5">
            <h3 className="text-[18px] font-semibold text-[#2e3240] tracking-[-0.36px] mb-1">SEO issues per page</h3>
